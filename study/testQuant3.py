...
IDS = ['601225.XSHG', '300411.XSHE', '002229.XSHE', '600503.XSHG', '002871.XSHE']
...
def plot_k(ID, beginDate, endDate, Type='d'):
    ...
    for i in range(len(quotes)):
        ...
        if abs(tmpDif - tmpDea) <= MAX_DIF:
            ...
            # singal_tmp.append(
            #                    {"aaa": tmpDif - tmpDea, "tradeDate": trade_date, "dif_value": tmpDif, "dea_value": tmpDea,
            #                     "closePrice": close_price, "openPrice": open_price, "highestPrice": high_price,
            #                     "lowestPrice": low_price, "turnoverRate": vol})
            aaa = tmpDif - tmpDea
            # 过滤掉下川的情况：dif 小于 Dea @todo
            # if aaa >=0 :
            # 来过滤掉当前的位置的下一个位置dif也需要大于dea
            idx = -len(quotes) + i - 1
            ...
            if idx <= 0 and aaa >= 0:
                ...
                if dif2 <= dea2:
                    ...
                    singal_tmp.append(
                        {"aaa": tmpDif - tmpDea, "tradeDate": trade_date, "dif_value": tmpDif, "dea_value": tmpDea,
                         "closePrice": close_price, "openPrice": open_price, "highestPrice": high_price,
                         "lowestPrice": low_price, "turnoverRate": vol, 'singal': 'buy', 'singal_reason': '极值，背离'})

                    ## 把DEA作为日期的标注点 @todo
                    # singal.append(tmpDea)
                    # singal_tmp.append({'tradeDate:':trade_date ,'dif_value:':tmpDea})
        else:
            singal.append(0)
